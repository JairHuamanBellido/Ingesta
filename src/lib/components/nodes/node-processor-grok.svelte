<script lang="ts">
	...
	function updateFieldValue(
		fieldKey: string,
		newValue: string | boolean | Array<string> | { field: string; regex: string }[]
	) {
			...
			<div class="space-y-2">
				...
				{#if field.type === 'object'}
					<div class="space-y-1">
						<div class="flex items-center justify-between">
							...
							<Button
								...
								onclick={() => {
									if (field.value) {
										updateFieldValue(field.key, [
											...(field.value as { field: string; regex: string }[]),
											{ field: '', regex: '' }
										]);
									} else {
										updateFieldValue(field.key, [{ field: '', regex: '' }]);
									}
								}}
							>
								<Plus />
						...
						<div class="flex flex-col space-y-4">
							{#each field.value as { field: string; regex: string }[] as pattern, index}
								<div class="flex items-center space-x-2">
									<Input
										oninput={(e) => {
											updateFieldValue(
												field.key,
												(field.value as { field: string; regex: string }[]).map((p, i) =>
													i === index ? { ...p, field: e.currentTarget.value } : p
												)
											);
										}}
										value={pattern.field}
									...
									<Input
										oninput={(e) =>
											updateFieldValue(
												field.key,
												(field.value as { field: string; regex: string }[]).map((p, i) =>
													i === index ? { ...p, regex: e.currentTarget.value } : p
												)
											)}
										value={pattern.regex}
										class={'bg-card'}
									...
									<Button
										...
										onclick={() => {
											updateFieldValue(
												field.key,
												(field.value as { field: string; regex: string }[]).filter(
													(_, i) => i !== index
												)
											);
										}}
